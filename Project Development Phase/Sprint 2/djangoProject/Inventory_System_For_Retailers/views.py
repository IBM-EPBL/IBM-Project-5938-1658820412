    ...
    if request.user.is_authenticated:
        ...
        if request.method == 'POST':
            ...
            if user is not None:
                ...
                return render(request, 'Inventory_Management_For_Retailers/login.html', {"error_message": "Invalid Credentials"})
        else:
...
def register(request):
    if request.method == 'POST':
        ...
        if User.objects.filter(username=username).exists():
            return render(request, "Inventory_Management_For_Retailers/register.html", {'error_message': "User already Exists"})
        elif password1 != password2:
            return render(request, "Inventory_Management_For_Retailers/register.html", {'error_message': "Password Mismatched"})
        else:
...
def addStock(request):
    if not request.user.is_authenticated:
        return redirect("/login", error_message="Login pannala bro neenga!")
...
def removeStock(request):
    ...
    if request.method == 'POST':
        try:
            ...
            return render(request, "Inventory_Management_For_Retailers/removeStock.html", {'Stocks': Stocks, 'error': "select a Stock"})
        else:
            Stock_To_Be_Removed = Stock.objects.get(id=Selected_Stock_id)
            Stocks = Stock.objects.all()
            if Selected_No_Of_Stocks < 0: return render(request, "Inventory_Management_For_Retailers/removeStock.html",
                                                        {'error_message': "Enter a Positive Value!", 'Stocks': Stocks})
            ...
            if Stock_To_Be_Removed.No_of_stocks < Selected_No_Of_Stocks:
                ...
                if Stock_To_Be_Removed.No_of_stocks == 0: Stock_To_Be_Removed.delete()
                return redirect('/')
    Stocks = Stock.objects.all()
